...
public class StaticBillboardChange : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (angle >= 225.0f && angle <= 315.0f) // 전면
            ChangeSprite(0);
        else if (angle < 225.0f && angle > 135.0f) // 우면
            ChangeSprite(1);
        else if (angle <= 135.0f && angle >= 45.0f) // 후면
            ChangeSprite(2);
        else if ((angle < 45.0f && angle > 0.0f) || (angle > 315.0f && angle < 360.0f)) // 좌면
            ChangeSprite(3);
    }
